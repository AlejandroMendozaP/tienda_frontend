...
export default {
  data() {
    return {
      ...
      ciudades: [],
      ordenes: []  // Añadido para almacenar las órdenes
    };
  },
  computed: {
    ...mapGetters(['getUser']),
    ...
    ordenesFiltradas() {
      return this.ordenes.filter(orden => orden.clienteId === this.user.id);
    }
  },
  methods: {
    ...mapActions(['logout']),
    ...
    async fetchOrdenes() {
      try {
        const response = await axios.get('http://localhost:3000/orden-compra');
  ...
  mounted() {
    ...
    this.fetchCiudades();
    this.fetchOrdenes(); // Fetch orders when component is mounted
  }
};
</script>
